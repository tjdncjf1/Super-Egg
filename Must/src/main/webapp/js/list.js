	...
	$.ajax({
		...
		success: function(userItems){
			...
			$.each(userItems.jsonResult.data, function(i, item){
				...
				listItem += '<span>' + item[i].title + '</span>';
				listItem += '<div class="viewImg">';
				listItem += '<img src="' + item[i].image + '" class="image"><hr>';
				...
				$(listItem).click(function(){
					...
					$('#select-items').show();
		  				...
		  				selectItem += '<h3 style="margin: 0 auto">' + item[i].title + '</h3>';
		  				selectItem += '<img src="' + item[i].image + '" class="image" style="border: 2px solid yellow;" /><br>';
//		  				selectItem += '<span>희망가격: <input type="number" placeholder=' + item.jsonResult.data.wish_price + ' id="wPrice" /><input type="button" value="변경" id="wish_update" /></span><br>';
		  				selectItem += '<span>최저가격: <input type="text" value=' + item[i].min_price + ' readonly="readonly" /></span>';
